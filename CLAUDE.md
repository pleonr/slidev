...
- **pnpm quirks**: `.npmrc` sets `shamefully-hoist=true` and `auto-install-peers=true`, required for Slidev's dependency resolution across the workspace. `neverBuiltDependencies` in the root `package.json` (`esbuild`, `playwright-chromium`, `vue-demi`) skips their postinstall build steps. If a deck's install/build breaks, check that its own `pnpm-lock.yaml` is in sync with its `package.json` — deck-level lockfile drift has been the recurring cause of build fixes in this repo's history.
